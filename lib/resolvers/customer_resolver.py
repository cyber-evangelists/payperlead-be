...
async def create_customer(customer: CreateCustomerInput) -> Customer:
    try:
        ...
        db_user = await CustomerEntity.find_one(
            CustomerEntity.email.value == customer.email
        )
        if db_user:
            raise Exception("Customer email already registered")
        ...
        inserted_customer = await CustomerEntity.find_one(
            CustomerEntity.email.value == customer.email
        )
